...
import {SafeAreaView, ImageBackground, FlatList,ScrollView, ActivityIndicator, RefreshControl} from 'react-native';
...
import { DuplicateReceipt, formatNumberWithCommas, numberToWords, numberToWordsRep } from '../../../constants/categories';
import { setReceiptList, setToUpdate } from '../../../store/invoices';
...
const ReceiptPdf = () => {  
  ...
  const handleNavigate = () => {
    ...
    dispatch(setReceiptList(receipt))  
  } 
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const targetInvoice = sortedGeneratedInvoice.find(invoice => invoice.uid === receiptList.uid);
      console.log('targetInvoice',targetInvoice)
      dispatch(fetchReceipt(user?.uid))  
      setReceipt(targetInvoice) 
    } catch (error) {
  ...
  }, [navigation,user, dispatch, receiptList?.uid]);
...
const DuplicateReceipt3 = async (receipt) => {
            ...
            {receipt?.CheckNumber?.trim() !== '' && (
              <View>
                ...
                <Button style={styles.button} type="blue" onPress={() => createPDF()}>
                    ...
                    <Button style={styles.button} type="blue" onPress={() => DuplicateReceipt3(receipt)}>
...
export default React.memo(ReceiptPdf)
